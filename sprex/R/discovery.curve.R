...
#' @param max.x the maximum number of samples to calculate the curve for. Defaults to the
...
#' 
#' @references Colwell, R.K., A. Chao, N.J. Gotelli, S.-Y. Lin, C.X. Mao, R.L. Chazdon,  
...
discovery.curve <- function(f, f0.func, max.x = sum(f * 1:length(f)), n.pts = 100, ci = 0.95, ...) {
  ...
  s.ind.ci <- s.ind[, "s.ind"] + qnorm((1 - ci) / 2) * cbind(s.ind[, "sd.s.ind"], -s.ind[, "sd.s.ind"])
  ci.poly <- cbind(x = c(n.seq, rev(n.seq)), y = c(s.ind.ci[, 1], rev(s.ind.ci[, 2])))
  i <- which(s.ind[, "m"] <= f.stats["n"])
  rarefact.line <- cbind(x = s.ind[i, "m"], y = s.ind[i, "s.ind"])
  extrap.line <- cbind(x = s.ind[-i, "m"], y = s.ind[-i, "s.ind"])
  
  d <- list(f.stats = f.stats, s.ind = s.ind, s.ind.ci = s.ind.ci, ci.poly = ci.poly,
       rarefact.line = rarefact.line, extrap.line = extrap.line)
